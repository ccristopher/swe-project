...
export function FinishedBookCard({ author, imageSrc, onClick, title }: FinishedBookCardProps) {
  return (
    <Card
      className={`gap-0 rounded-[1.9rem] border-0 bg-surface-container-low p-3 shadow-[0_14px_30px_var(--card-shadow)] transition-shadow duration-150 hover:shadow-[0_18px_34px_var(--card-shadow)] ${onClick ? 'cursor-pointer' : ''}`}
      onClick={onClick}
    >
      <div
