...
type AppCollections = {
  ...
  pets: Collection<any>;
  books: Collection<any>;
  items: Collection<any>;
};

let schemasPromise: Promise<AppCollections> | undefined;
  ...
  if (!schemasPromise) {
    schemasPromise = (async () => {
      ...
      const users = await db
        .createCollection("users", userSchema)
        .catch(() => db.collection("users"));
      const pets = await db
        .createCollection("pets", petsSchema)
        .catch(() => db.collection("pets"));
      const books = await db
        .createCollection("books", booksSchema)
        .catch(() => db.collection("books"));
      const items = await db
        .createCollection("items", itemsSchema)
        .catch(() => db.collection("items"));
